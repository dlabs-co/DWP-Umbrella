...
void loop () 
{
  ...
  runEvery(5000)
  {
    ...
    lcd.print("               ");
    lcd.setCursor(0,0);
    lcd.print(strFecha(now));
    lcd.setCursor(0,1);
    lcd.print(strHora(now));
    Serial.println(strFecha(now));
    Serial.println(strHora(now));
...
String strFecha(const RtcDateTime& dt)
{
  String fecha = String(dt.Day()) + String("/") + String(dt.Month()) + String("/") +String(dt.Year());
  return fecha;
}

String strHora(const RtcDateTime& dt)
{
